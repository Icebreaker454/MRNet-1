...
def main(data_dir):
    ...
    train_df = pd.merge(train_abnormal_df, train_acl_df, on="case").merge(
        train_meniscus_df, on="case"
    )
    ...
    valid_df = pd.merge(valid_abnormal_df, valid_acl_df, on="case").merge(
        valid_meniscus_df, on="case"
    )
