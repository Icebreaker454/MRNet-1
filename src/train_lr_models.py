...
def main(data_dir: str, models_dir: str, backbone: BackboneType = None):
    ...
    for condition in conditions:
        ...
        for plane in planes:
            checkpoint_pattern = glob(
                f"{models_dir}/{backbone}_*{plane}*{condition}*.pt"
            )
            try:
    ...
    with tqdm(total=len(axial_loader)) as pbar:
        for (axial_inputs, labels), (coronal_inputs, _), (sagittal_inputs, _) in zip(
            axial_loader, coronal_loader, sagittal_loader
        ):

            axial_inputs, coronal_inputs, sagittal_inputs = (
                axial_inputs.to(device),
                coronal_inputs.to(device),
                sagittal_inputs.to(device),
            )
